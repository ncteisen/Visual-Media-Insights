...
class Show:
    def __init__(self, show_metadata, season_list):
        ...
        self.episode_count = sum(
            map(lambda season: len(season.episode_list), season_list))

    def __str__(self):
        return "Show[title={title}, imdb_id={imdb_id}, season_count={season_count}, episode_count={episode_count}]".format(
            title=self.title, imdb_id=self.imdb_id, season_count=self.season_count, episode_count=self.episode_count)
