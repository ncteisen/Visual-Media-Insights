...
class ShowInsights:
    ...
    def all_episodes(self):
        return list(itertools.chain.from_iterable(
            [season.episode_list for season in self.show.season_list]))

    @property
    def slope(self):
        return linregress([i for i in range(self.show.episode_count)], [
                          e.score for e in self.all_episodes]).slope
...
if __name__ == "__main__":
    ...
    for season in show.season_list:
        ...
        best = insights.best_episode
        print(
            "  best:  ({number}/{episode_count}) {title} ({score}/10)".format(
                number=best.number,
        ...
        worst = insights.worst_episode
        print(
            "  worst: ({number}/{episode_count}) {title} ({score}/10)".format(
                number=worst.number,
                episode_count=season.episode_count,
                title=worst.title,
                score=worst.score))
